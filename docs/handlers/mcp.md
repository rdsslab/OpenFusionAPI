...
**Code Config**:
Typically requires no specific code in the editor if using the default factory, as the logic is handled by `server_mcp` factory which aggregates _other_ endpoints.

**Enabling Tools**:
To make _other_ endpoints visible to this MCP server, you must configure the **MCP** settings in their respective "Advanced" tabs:
